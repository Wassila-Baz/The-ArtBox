...
<body>
    ...
    <!-- Contenu principal -->
    <main>
        <div id="liste-oeuvres" class="galerie">
            <?php 
            // Vérifie si $oeuvres est défini et est un tableau avant d'itérer dessus
            if (isset($oeuvres) && is_array($oeuvres)) {
                foreach ($oeuvres as $oeuvre) {
                    // Assurez-vous que $oeuvre est un tableau avant d'accéder à ses éléments
                    if (is_array($oeuvre) && !empty($oeuvre['titre'])) {
            ?>
            <article class="oeuvre">
                <a href="detail-oeuvre.php?id=<?php echo htmlspecialchars($oeuvre['id']); ?>">
            ...
            <?php 
                ...
                // Gérer le cas où $oeuvres n'est pas défini ou est vide
                echo "Aucune œuvre trouvée.";
            }
            ?>
            
            <!-- Exemples supplémentaires -->
            <!-- Vous pouvez ajouter des articles supplémentaires ici -->
